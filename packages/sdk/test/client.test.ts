...
describe('NestorClient', () => {
  it('builds prefixed URLs and unwraps the ApiResponse envelope', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        jsonResponse(ok({ id: '1', username: 'alice', roles: [], permissions: [] })),
      );
  ...
  it('throws NestorApiError carrying code/traceId on business failure', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ code: 40001, message: 'bad', data: null, timestamp: 0, traceId: 't-9' }),
      );
  ...
  it('auto-refreshes once on an expired access token, then retries', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse(ok({ accessToken: 'a2', refreshToken: 'r2', tokenType: 'Bearer' })),
      )
      // 3) retried profile -> success with the refreshed token
      .mockResolvedValueOnce(
        jsonResponse(ok({ id: '1', username: 'bob', roles: [], permissions: [] })),
      );
    const store = new MemoryTokenStore();
    store.set(TOKENS);
  ...
  it('uploads a file as multipart/form-data under the `file` field', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse(ok({ id: 'f1', originalName: 'a.txt' })));
    const client = new NestorClient({ baseUrl: 'http://x', fetch: fetchMock });
    client.setTokens(TOKENS);
  ...
  it('exposes absolute raw file URLs and serialises list query params', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        jsonResponse(ok({ list: [], total: 0, page: 2, pageSize: 5, totalPages: 0 })),
      );
    const client = new NestorClient({ baseUrl: 'http://x', fetch: fetchMock });
    client.setTokens(TOKENS);
